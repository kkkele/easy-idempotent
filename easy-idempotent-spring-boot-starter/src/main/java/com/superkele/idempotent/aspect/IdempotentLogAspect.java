...
public class IdempotentLogAspect {
    ...
    public Object idempotentHandler(ProceedingJoinPoint joinPoint, Idempotent idempotent) {
        ...
        try {
            ...
            printLog(joinPoint, "新建幂等标识{} ,{}s内不可重复消费", () -> repeatSubmit.getKey(), () -> interval / 1000);
            resultObj = joinPoint.proceed();
            //后置处理
            repeatSubmit.afterHandler(SpelUtils.parseResult(idempotent.clean(), resultObj, Boolean.class));
            printLog(joinPoint, "幂等标识{},执行后置处理器成功", () -> repeatSubmit.getKey(), () -> interval / 1000);
        } catch (RepeatSubmitException e) {
            printLog(joinPoint, "幂等标识{} [{}]", () -> repeatSubmit.getKey(), () -> properties.getRestApi().getMessage());
            throw new RepeatSubmitException(properties.getRestApi().getMessage());
        } catch (MqRepeatConsumerException e) {
            printLog(joinPoint, "幂等标识{} [{}]", () -> repeatSubmit.getKey(), () -> properties.getMq().getMessage());
            return null;
        } catch (Throwable e) {
